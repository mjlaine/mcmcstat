...
static char user_routine_name[20][64];
 ...
 #define ROUTNAME DPCHEZ
#else
 #define FFUNCALL 
 #define FFUNPTR *
 #define ROUTNAME dpchez_
#endif
...
/* Check the number of input and output arguments */
  if (gm_nin != 8) {
    mexErrMsgTxt("Needs 8 input parameters");
  } else if ((gm_nout != 8)) {
  ...
  spline = (int *) mxCalloc(gm_l,sizeof(int));
  for (gm_i=0;gm_i<gm_l;gm_i++) {
    spline[gm_i] = (int) (mxGetPr(gm_pin[4]))[gm_i];
  }
  w = mxGetPr(gm_pin[5]);
  gm_l = mxGetM(gm_pin[6])*mxGetN(gm_pin[6]);
  lw = (int *) mxCalloc(gm_l,sizeof(int));
  for (gm_i=0;gm_i<gm_l;gm_i++) {
    lw[gm_i] = (int) (mxGetPr(gm_pin[6]))[gm_i];
  }
  gm_l = mxGetM(gm_pin[7])*mxGetN(gm_pin[7]);
  ierr = (int *) mxCalloc(gm_l,sizeof(int));
  for (gm_i=0;gm_i<gm_l;gm_i++) {
    ierr[gm_i] = (int) (mxGetPr(gm_pin[7]))[gm_i];
  }

 /* Ready to call the routine */
  mexSetTrapFlag(0);
#ifdef MSC
  DPCHEZ(n,x,f,d,spline,w,lw,ierr);
#else
  dpchez_(n,x,f,d,spline,w,lw,ierr);
#endif
  ...
  gm_l = gm_n*gm_m;
  for (gm_i=0;gm_i<gm_l;gm_i++) {
     (mxGetPr(gm_pout[4]))[gm_i] = (double) spline[gm_i];
  }
  mxFree(spline);
  ...
  gm_l = gm_n*gm_m;
  for (gm_i=0;gm_i<gm_l;gm_i++) {
     (mxGetPr(gm_pout[6]))[gm_i] = (double) lw[gm_i];
  }
  mxFree(lw);
